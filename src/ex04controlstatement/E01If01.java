...
//		}
		...
		//경우레 따라 무조건 실행되는 if문도 필요함
		if(true) {
			System.out.println("무조건 실행되는 if문입니다.");
		}
		
		/*
		 if(조건식);=> 조건식에 상관없이 다음 문장이 실행되는 형태로 세미콜론은 문장의 끝을
		 나타내므로 사용에 주의해야한다.
		 */
		if(num%2!=0); {
		...
		/*
		 System.in.read()
		 	:사용자에게 하나의 문자를 입력받을때 사용하는 함수(메소드)로 입력한 문자의 아스키코드값을
		 	반환한다. 해당 함수는 IOException을 기본적으로 가지고 있어서
...
//		System.out.println("입력한문자:"+(char)asciicode);
	 ...
	 */
		...
		/*
		 아스키 코드값을 알고있을때 구현방법
		 '0'=>48
		 '98'=>57이므로 해당 구간에 있다면 숫자로 판단할수 있다.

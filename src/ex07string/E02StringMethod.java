...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		//2] charAt(인덱스):문자열에서 특정 index에 해당하는 문자 하나를 반환 한다.index는 0부터 시작
		System.out.println("str의 두번째 문자:"+str1.charAt(1));
		System.out.println("str의 두번째 문자:"+str2.charAt(1));
		...
		/*
		//4]compareTo():두 문자열의 첫번째 문자부터 순차적으로 비교하면서 
					 	앞의 문자열의 아스키 코드가 크면양수를 반환
					  	뒤의 문자열의 아스키 코드가 크면음수를 반환
					  	같으면 0을 반환한다.
		*/
		String str3="A";
		String str4="B";
		...
		//5]concat():두개의 문자열을 연결할때 사용한다. +기호와 동일한 역할을 한다.
		System.out.println("JAVA".concat("WORLD").concat(" GO"));
		System.out.println("JAVA"+"WORLD"+" GO");
		
		/*
		 6]contains()
		 	:문자열에 특정 문자열이 포함되어 있는지를 판단하여 포함되었으면 true를 반환한다.
		 */
		...
		/*
		 ...
		 8]startsWith()
		 	:특정 문자열로 시작하면서 true를 반환한다.
		 */
		System.out.println("www.daum.net".endsWith("net"));//true
		System.out.println("www.naver.com".endsWith("http"));//false
		...
		/*
		 시나리오] 주민등록번호를 index Of를 사용하여 성별을 구분하는 프로그램을 작성하시오
		 */
		//'_'(하이픈)의 위치를 찾아+1하면 성별에 해당하는 문자를 얻을 수 있음.
		juminNum="120403-3012345";
		int index= juminNum.indexOf("_")+1;
		...
		System.out.println(str1.replace("java","KOSMO"));//변경
		
		/*
		 13]split()
		 	:문자열을 구분자를 통해 분리해서 String타입 배열로 반환한다.
		...
		/*
		 14]substring()
		 	...
		 	사용1:substring(index)->이 경우 index부터
		 		문자열의 끝까지를 잘라서 반환함.
		 	사용2:substring(index1,index2)->인덱스에 포함되는
		 		부분만 잘라서 반환함
		 */

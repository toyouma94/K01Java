...
class NotThread{
	...
	void notThreadMethod() {
		for(int i=1;i<=10;i++) {
			System.out.println(String.format("%s]i=%dd",title,i));
		};
	}
}
/*
...
class YesThread extends Thread{
	...
	/*
	 ...
	 해당 쓰레드의 이름을 지정할 수 있다. 지정된 이름은 getName()
	 을 통해 반환할 수 있다.
	 */
	...
	void threadMethod()throws InterruptedException{
		for(int i=1;i<=10;i++) {
			...
			System.out.println(String.format("%s]i=%dd",getName(),i)
		);
		sleep(2000);
		}
	}
...
/*
 종속쓰레드(Daemon Thread0
 	:모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다.
 	주로 프로그램의 보조역할로 배경음악, 로그 자동저장등의 업무를 처리한다
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		nt2.notThreadMethod();
		/*
		 1번객체가 종료되면 2번객체가 실행되는 일반적인 형태로 실행된다.
		 */
		
		/*
		 독립쓰레드 실행(Non daemon thread)생성 및 실행
		 */
		YesThread yt1=new YesThread("1st 쓰레드");//생성자를 통해 이름 부여
		//yt1.setName("첫번째쓰레드");//쓰레드의 이름을 지정하는 두번째 방법
		yt1.start();

...
/*
 Thread(쓰레드)
 -쓰레드의 생성은 Thread클래스를 상속받는 것 부터 시작한다.
 -해당 방법은 쓰레드로 생성할 클래스가 다른 클래스를 상속받지 않아도
 될때 사용하는 방식이다.
...
class ShowThread extends Thread
{
	...
	/*
	 run()메소드는 쓰레드의main()메소드엥 해당한다.
	 Thread클래스의 run()메소드를 오버라이딩 한것으로 해당
	 메소드는 직접 호출하면 안되고,start()를 통해 간접적으로 호출해야한다.
	 만약 직접 호출하면 단순한 실행만 될뿐 쓰레드가 생성되지 않으므로 주의해야한다.
	 */
	@Override
	public void run() {
		for(int i=1;i<=100;i++) {
			...
			try {
				/*
				 ...
				 1000분의 1초 단위로 기술하게 되므로 아래는 0.01초 만큼
				 Block시키게 된다
				 */
				sleep(10);
			}

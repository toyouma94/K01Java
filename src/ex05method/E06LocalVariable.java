...
/*
 ...
 -기본자료형은 스택(stack)이라는 메모리 공간을 사용ㅇ한다.
...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		boolean scope=true;
		/*
		 main지역에서 num을 선언하면 그보다 작은 if문의 지역에서는 같은 이름의
		 변수를 선언할 수 없다.
		...
		if(scope) {
			...
			int num=5;
			System.out.println("첫번째 if 문의 else지역:"+num);
		}
		/*
		 if문의 블럭에서 선언된 변수 num은 해당 블럭을 벗어나면 메모리에서 소멸된다.
		 따라서 아래 num을 선얼할 수 있다.
		 */
		int num=100;
	...
	/*
	 main에서 선언한 변수 num이 메모리에 아직 남아있지만, simpleFunc()메소드는 
	 서로 다른 지역에 정의되었음으로 같은 이름의 변수를 선언할 수 있다.
	 */

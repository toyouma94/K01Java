...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		set.add(strObject2);
		set.add(dateObject);
		set.add(number);//<-new Integer(number) 생성후 저장
		set.add(teacher);
		...
		/*5-1]기본 클래스형 객체 중복저장
			:기본클래스의 경우에는 별도의 오버라이딩 처리없이
		  	중복이 제거된다. 따라서 아래의 경우에는 false가 반환된다.
		 */
		...
		/*5-2]개발자가 정의한 객체 중복저장
			...
			오버라이딩ㅇ 하지 않으면 중복저장이 허용된다. 아래의 경우
			저장에 성공하여 true가 반환된다.
		 */
		...
		//6]저장된 객체 출력 :순서 없이 저장되므로 출력도 순서를 지정할 수 없다.
		Iterator itr=set.iterator();
		while(itr.hasNext()) {
			Object object=itr.next();
			
			if(object instanceof String) {
				System.out.println("Strring타입:"+object);
			}
		...
		//1-7]존재여부확인
		System.out.println(set.contains(strObject1)?"strOnject있다":"strOnject없다");
		System.out.println(set.contains("JaVa")?"JaVa있다":"JaVa없다");
		
		//1-8]삭제
		//	:Set은 index가 없으므로 참조값으로만 삭제가능
		System.out.println(set.remove(strObject2)?"strOnject삭제성공":"strOnject삭제실패");

	...
	public static void main(String[] args) {
		try {
			...
			if(con!=null) {
				...
				/*
				 3]쿼리문작성
				 	:쿼리문을 작성할때 주의할 점은 줄바꿈을 할떄 앞뒤로
				 	스페이스바를 삽입하는것이 좋다. 그렇지 않으면 문장이
				...
				/*
				 4]쿼리문을 Oracle로 전송하기위한 Statement계열의 인터페이스를
				 사용한다. 해당 객체는 주로 정적쿼리를 실행할때 사용된다.
				 */
				Statement stmt=con.createStatement();
				
				/*
				 5]Statement객체를 사용하여  Oracle로 작성된 쿼리문을 전송하고 실행한다.
				 실행후 결과를 반환하면 ResultSet객체를 통해 받는다.
				 */
				...
				while(rs.next()) {
					/*
					 ...
					 콘솔에 내용을 출력한다.
					 getXXX()계열의 함수는 문자,숫자,날짜를 사용하기 위해
					 생성되어있다.인자값으로는 컬럼의 순서에 따른 인덱스를
					 쓸수도 있고,컴럼명을 쓸수도 있다.
					 */

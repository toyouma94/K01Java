...
/*
 ...
 -main()메소드가 실행되기전에 이미 초기화되므로 main메소드 실행시 별도의 선언없이 변수를 사용할 수 있다.
 	
 접근방법
 	-클래스내부
 		변수명으로 접근 가능(일반적인 멤버변수와 동일함)
 	-클래스외부
 		참조변수.변수명or클래스명.변수명 둘다 가능함
 */
class MyStatic{
	/*
	...
	int instanceVar;
	/*
	 정적멤버변수]객체를 생설할 필요없이 클래스명으로 바로 접근이 가능하다.
	 	프로그램 시작시 이밈 메소드 영역에 초기화된다.
	 */
	static int staticVar;
	/*
	 인스턴스형 멤버메소드]인스턴스형 메소드에서는 모든 멤버를 사용할 수 있다.
	 인스턴스형, 정적변수 둘다 접근 가능함.
	 */
	void instanceMethod() {
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 방법2:일반적인 멤버변수처럼 참조변수를 통해 접근하는것도 가능하다
		 	단, 이경우는 정적변수를 일반적인 멤버변수로 착각 할 수 있으므로
		 	자바에서 권장하지 않는다.
		 */
		...
		/*
		 ...
		 객체별로 각각 저장된다. 따라서 출력과는 100,1000이 된다.
		 */
		ms1.instanceVar=100;
